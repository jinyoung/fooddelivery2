...
class MenuDB():
    def __init__(self):
        self.engine = create_engine('sqlite:///Menu_table.db', echo=True, connect_args={'check_same_thread': False})
    ...
    def find_by_id(self, id:int):
        ...
        session = DBsession()
        try:
            query = session.query(Menu).filter_by(id=id).one()
    ...
    def update(self, id, request):
        ...
        try:
            query = session.query(Menu).filter(Menu.id==id).first()
